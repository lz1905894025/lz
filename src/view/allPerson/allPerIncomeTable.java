 ...
 * 这里是显示收入表信息的
 */
public class allPerIncomeTable extends JDialog {
     ...
     * Create the dialog.
     */
    public allPerIncomeTable() throws SQLException {

        titles=new Vector<>();
        Collections.addAll(titles,"用户名","姓名","性别","其他收入信息","总收入","工资","其他收入");
    ...
    public MyDefaultTableModel getTableModel() throws SQLException {
        ...
        Vector<Vector> rows= selectIncome.Income();
        for (Vector row:rows)
            model.addRow(row);
        return model;
